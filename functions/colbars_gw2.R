colbars_gw2 <- function(df2, colscheme = 2){
  ...
  if(any(duplicated(as.character(unlist(uni))))) { # FALSE
    ...
    if(colscheme != 2)
    { colors <- c("peachpuff3", "tomato", "yellowgreen", "burlywood")[1:no.of.levels]
    } else {
      colors <-c("plum4", "yellow2", "dodgerblue2", "violetred4", "hotorange", "brown" )[1:no.of.levels]
    }
    ...
    colours[[1]] <- c("red", "green", "dodgerblue2", "violetred4", "yellow2", "plum4", "palevioletred4")
    colours[[2]] <- c("hotpink", "orange", "deepskyblue", "firebrickred", "brown4", "orchid4", "pink2")
    colours[[3]] <- cm.colors(10)
